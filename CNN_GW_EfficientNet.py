...
lrr = ReduceLROnPlateau(monitor = 'val_loss', factor = 0.01, patience = 1, mode = 'min', min_lr = 0.00001) #factor = 0.01   min_lr = 0.00001
...
def Model():
    ...
    efficientnet_layers = EfficientNetB0(include_top = False, input_shape = (224, 224, 3), weights = 'imagenet', pooling = 'avg')
    sgd = SGD(lr = 0.001, momentum = 0.9, nesterov = True) #False
    model = Sequential()

    
    model.add(inputs)
    model.add(tf.keras.layers.experimental.preprocessing.Resizing(224, 224))
    model.add(keras.layers.Conv2D(3, 3, activation = 'relu', padding = 'same', kernel_regularizer = keras.regularizers.l2(0.0005), use_bias = True))
